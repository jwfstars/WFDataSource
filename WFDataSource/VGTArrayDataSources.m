...
#import "VGTArrayDataSources.h"

@interface VGTArrayDataSources()<UITableViewDataSource, UICollectionViewDataSource, UICollectionViewDelegate, UITableViewDelegate,UICollectionViewDelegateFlowLayout>
...
@implementation VGTArrayDataSources
...
- (void)insertNewSectionItemsOrItems:(NSArray *)sectionItemsOrItems atIndexPath:(NSIndexPath *)indexPath type:(NSInteger)type
{
    if (self.sectionItems) {
        //只包含VGTEmpty或者VGTError的情况
        NSMutableArray *sectionSubArray = [self.sectionItems.lastObject valueForKey:[self sectionPropertiesMap][SECTION_SUBARRAY_NAME]];
        if ([sectionSubArray.lastObject isKindOfClass:[VGTError class]] || [sectionSubArray.lastObject isKindOfClass:[VGTEmpty class]]) {
            if (type == 0) {
                [self reloadWithItems:sectionItemsOrItems];
            }else {
                [self reloadWithSectionItems:sectionItemsOrItems];
            }
            return;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([item isKindOfClass:[VGTEmpty class]] || [item isKindOfClass:[VGTError class]]) {
        [tableView registerNib:[UINib nibWithNibName:@"VGTEmptyCell" bundle:nil] forCellReuseIdentifier:@"VGTEmptyCell"];
        [tableView registerNib:[UINib nibWithNibName:@"VGTErrorCell" bundle:nil] forCellReuseIdentifier:@"VGTErrorCell"];
    ...
    if ([item isKindOfClass:[VGTError class]] || [item isKindOfClass:[VGTEmpty class]]) {
        VGTEmptyCell *tableViewCell = (VGTEmptyCell *)cell;
...
{
    id item = [self itemAtIndexPath:indexPath];
    if ([item isKindOfClass:[VGTError class]]) {
        return self.tableViewRealHeight * 2 / 3;
    }else if ([item isKindOfClass:[VGTEmpty class]]) {
        return self.tableViewRealHeight * 1 / 2;
    }else {
...
- (void)handleEmptyWithMessage:(NSString *)message imageName:(NSString *)imageName
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"VGTEmptyCell" bundle:nil] forCellReuseIdentifier:@"VGTEmptyCell"];
    [self reloadWithItems:@[emptyObject]];
}

- (void)handleErrorWithMessage:(NSString *)message imageName:(NSString *)imageName
{
    VGTError *errorObject = [VGTError new];
    errorObject.message = message;
    errorObject.imageName = imageName;
    [self.tableView registerNib:[UINib nibWithNibName:@"VGTErrorCell" bundle:nil] forCellReuseIdentifier:@"VGTErrorCell"];
    [self reloadWithItems:@[errorObject]];
}
@end
